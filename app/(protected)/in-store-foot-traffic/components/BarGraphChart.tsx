...
function getCustomersByDate(transactions: Transaction[]) {
  ...
  transactions.forEach((transaction) => {
    ...
    if (!customersMap[dateKey]) {
      customersMap[dateKey] = 0;
    }

    customersMap[dateKey] += transaction.numPax || 1;
  });
...
export default function BarGraphChart({
  ...
  // Map sales per date
  const customersMapThisWeek = getCustomersByDate(transactionsSecondWeek);
  ...
  const chartData = secondWeekDays.map((day) => {
    ...
    const value = customersMapThisWeek[thisWeekKey] || 0;
